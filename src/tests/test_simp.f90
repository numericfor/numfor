program test_simps
  ...
  use integrate, only: simps, nf_rfunction, nf_cfunction
  implicit none
  ...
  a = 1.e-5_dp; b = 1._dp
  do i = 1, 5
    Int1 = simps(fquad451, a, b, i * N)
    err = abs((result - Int1) / result)
    print "(A)", 'simps = '//str(Int1)//" (Error="//str(err)//") with N = "//str(i * N)
  end do
  ...
  do i = 1, 5
    ...
    fun1%args = [w]
    Int1 = simps(fun1, a, b, 4 * N)
    err = abs((result - Int1) / result)
    print "(A)", 'simps = '//str(Int1)//" (w="//str(w)//", Error="//str(err)//") with N = "//str(4 * N)
  end do
  ...
  do i = 1, 5
    w = 25_dp * i
    fun2%args = [w]
    Int2 = simps(fun2, a, b, 10 * N)
    err = abs((results(i) - Int2) / results(i))
    print "(A)", "maxima qags = "//str(results(i))//"  w = "//str(w)
    print "(A)", 'Aprox simps = '//str(Int2)//" Error="//str(err)//"   N = "//str(10 * N)
    ! print *, Int2
  end do
