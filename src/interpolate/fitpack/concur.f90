subroutine concur(iopt, idim, m, u, mx, x, xx, w, ib, db, nb, ie, de, ne, k, s,&
  ...
  !  ..local scalars..
  real(8) :: tol
  integer :: i, ib1, ie1, ja, jb, jfp, jg, jq, jz, j, k1, k2
  integer :: lwest, maxit, nmin, ncc, kk, mmin, nmax, mxx
  ! ..function references
  integer :: max0
  !  ..
...
70 jfp = 1
  ...
  jg = jb + nest * k2
  jq = jg + nest * k2
  call fpcons(iopt, idim, m, u, mxx, xx, w, ib, ie, k, s, nest, tol, maxit, k1, k2, &
    &n, t, ncc, c, fp, wrk(jfp), wrk(jz), wrk(ja), wrk(jb), wrk(jg), wrk(jq), iwrk, ier)
