subroutine fpopdi(ifsu, ifsv, ifbu, ifbv, u, mu, v, mv, z, mz, z0, dz,&
  ...
  !
  !  ..scalar arguments..
  integer :: ifsu, ifsv, ifbu, ifbv, mu, mv, mz, nu, nv, nuest, nvest, nc, lwrk
  real(8) :: z0, p, step, fp
  !  ..array arguments..
  integer :: ider(2), nru(mu), nrv(mv), iopt(3)
  real(8) :: u(mu), v(mv), z(mz), dz(3), tu(nu), tv(nv), c(nc), &
    &fpu(nu), fpv(nv), wrk(lwrk)
  !  ..local scalars..
...
5 if (step <= 0. .or. sq <= 0.) return
  ...
  nr(1) = 1
  delta(1) = step
10 if (iop0 == 0) go to 20
  if (ider(2) /= 0) go to 20
