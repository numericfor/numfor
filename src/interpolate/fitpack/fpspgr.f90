...
!  estimate these values by computing a least-squares spline
          ...
          if (idd(3) > 0) dr(4) = r1
          idd(4) = ider(4)
          if (ider(1) < 0 .or. ider(3) < 0) go to 30
          if (iopt(2) /= 0 .and. ider(2) == 0) go to 30
...
150                             call fpopsp(ifsu, ifsv, ifbu, ifbv, u, mu, v, mv, r, mr, r0, r1, dr, iopt,
                                ...
                                *nrv, wrk, lwrk)
                                if (step(1) < 0.) step(1) = -step(1)
                                if (step(2) < 0.) step(2) = -step(2)
                                if (ier == (-2)) fp0 = fp
...
!  find nplu, the number of knots we should add in the u-direction.
                                ...
                                npl1 = nplusu * 2
                                rn = nplusu
                                if (reducu > acc) npl1 = rn * fpms / reducu
...
180                             nplv = 3
                                ...
                                npl1 = nplusv * 2
                                rn = nplusv
                                if (reducv > acc) npl1 = rn * fpms / reducv
